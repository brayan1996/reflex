...
const d = new Date();
const fechaActualValue = `${d.getFullYear()}-${('0' + (d.getMonth() + 1)).slice(
  -2
)}-${('0' + d.getDate()).slice(-2)}`;

export const FormCreateCitas = ({ dateCalendar, setActiveIndex }) => {
  ...
  const tarificaCitaSelected = (tarifaCode) => {
    const operationTarifaCita = {
      ...
      mt: () => {
        adelanto.onChange(25);
        final.onChange(0);
        total.onChange(25);
      },
      r: () => {
        adelanto.onChange(50);
        final.onChange(0);
        total.onChange(50);
      },
    };
  ...
  const tarifaAdelantoSelected = (adelantoCOde) => {
    const operationsAdelantoCita = {
      a20: () => {
        adelanto.onChange(20);
        final.onChange(0);
        total.onChange(20);
      },
      a30: () => {
        adelanto.onChange(30);
        final.onChange(0);
        total.onChange(30);
      },
      r: () => {
  ...
  useEffect(() => {
    total.onChange(
      (parseInt(adelanto.value) || 0) + (parseInt(final.value) || 0)
    );
  ...
  useEffect(() => {
    if (dateCalendar)
      setTime((oldTime) => ({
        hora: oldTime.hora,
        fecha: changeFormateDate(dateCalendar),
      }));
  }, [dateCalendar]);
  ...
  useEffect(() => {
    const d = new Date();
    const fechaActual = `${d.getFullYear()}-${('0' + (d.getMonth() + 1)).slice(
      -2
    )}-${('0' + d.getDate()).slice(-2)}`;
    const horaActual = `${('0' + d.getHours()).slice(-2)}:${(
      '0' + d.getMinutes()
    ).slice(-2)}`;
  ...
  return (
    <div className='w-full form1'>
      ...
      <div className='w-full mt-10 mx-auto grid grid-cols-4 gap-4'>
        <TopNamedInput {...fecha} label='Fecha' />
        <TopNamedCombobox
          label='Hora'
          data={hoursDate}
          dataKey='value'
        ...
        <TopNamedInput
          ...
          onBlur={async (e) => {
            const name = (
              await Pacientes.requestAPatient(e.target.value)
            )?.data.shift()?.NOMBRE;
        ...
        />
        <TopNamedInput {...cliente} label='Cliente' />
        <TopNamedInput {...observacion} label='Observación' />
        <CheckedComponent label='cita' {...citaChecked} />
        <TopNamedCombobox
          label='Tarifa cita'
        ...
        />
        <TopNamedInput {...adelanto} label='Imp adelanto' />
        <TopNamedInput {...final} label='Imp final' />
        <TopNamedInput {...total} label='total' disabled />
      </div>
